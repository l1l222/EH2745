...
from EquipmentClass.TransformerClass import PowerTransformer, PowerTransformerEnd, RatioTapChanger, PetersenCoil, PhaseTapChangerAsymmetrical, TapChangerControl
...
from EquipmentClass.GeographicalRegionClass import GeographicalRegion, SubGeographicalRegion
from EquipmentClass.CurrentLimitClass import CurrentLimit
from EquipmentClass.CurveDataClass import CurveData
from EquipmentClass.EnergyConsumerClass import EnergyConsumer
from EquipmentClass.GeneratorClass import SynchronousMachine, GeneratingUnit
from EquipmentClass.ShuntCompensatorClass import LinearShuntCompensator
from EquipmentClass.LoadResponseCharacteristicClass import LoadResponseCharacteristic
from EquipmentClass.LimitClass import OperationalLimitSet, OperationalLimitType
from EquipmentClass.ReactiveCapabilityCurveClass import ReactiveCapabilityCurve
from EquipmentClass.RegulatingControlClass import RegulatingControl
...
for BBS in BusbarSection_:
    BBS.ppbus = pp.create_bus(network, name = BBS.name, vn_kv = float(BBS.nominalVoltage), type = 'b')

# Create bus for each CN not attahced to busbar
for cn in CN_notAttached_Busbar:
    cn.pp_bus = pp.create_bus(network, name = cn.name, vn_kv = float(cn.nominalVoltage), type = 'n')

# Register created bus with type b in the CNs attached to busbar
for cn in ConnectivityNode_:
    if cn.ID in list(CN_Busbar.keys()):
        cn.pp_bus = CN_Busbar[cn.ID].ppbus
    ...
    if len(path) == 3:
        ...
        if path[-1].CE_type == 'LinearShuntCompensator':
            ...
            for sh in LinearShuntCompensator_:
                if sh.ID == path[-1].ID:
                    sh.pp_shunt = pp.create_shunt(network, temp_cn, p_mw = 0, q_mvar = -0.96, name = sh.name)
                    
        if path[-1].CE_type == 'EnergyConsumer':
            
            for ec in EnergyConsumer_:
                if path[-1].ID == ec.ID:
                    ec.pp_EC = pp.create_load(network, temp_cn, p_mw = float(ec.p), q_mvar = float(ec.q), in_service = True, name = ec.name)
                             
        if path[-1].CE_type == 'SynchronousMachine':
            
            for sm in SynchronousMachine_:
                if path[-1].ID == sm.ID:
                    sm.pp_gen = pp.create_gen(network, temp_cn, p_mw = -float(sm.p), vm_pu = float(sm.ratedU), max_q_mvar = -float(sm.maxQ), 
...
for path in all_everything_stack:
    ...
    if len(path) == 5:
        ...
        if path[2].CE_type == 'PowerTransformer':
            ...
            for trans in PowerTransformer_:
                if trans.ID == path[2].ID:
                    trans.pp_trans = pp.create_transformer(network, hv, lv, name = trans.name, std_type = '25 MVA 110/20 kV')
                    break
        ...
        if path[2].CE_type == 'Breaker':
            ...
            for cb in Breaker_:
                if cb.ID == path[2].ID:
                    cb.pp_cb = pp.create_switch(network, temp_cn1.pp_bus, temp_cn2.pp_bus, et = 'b', type = 'CB', name = cb.name)
            
        if path[2].CE_type == 'ACLineSegment':
            
            for line in ACLineSegment_:
                if line.ID == path[2].ID:
                    line.pp_line = pp.create_line(network, temp_cn1.pp_bus, temp_cn2.pp_bus, length_km = 10, name = line.name, std_type = '48-AL1/8-ST1A 20.0')
  
#pp.plotting.simple_plot(network)
pp.plotting.to_html(network, 'plot_system2.html')
